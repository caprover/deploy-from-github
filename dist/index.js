...
async function createGitArchive(ref, cwd = process.env.GITHUB_WORKSPACE || process.cwd()) {
  ...
  try {
    ...
    return {
      path: archivePath,
      gitHash,
      cleanup: () => (0, import_promises.rm)(directory, { recursive: true, force: true })
    };
  } catch (error) {
...
var CapRoverClient = class {
  ...
  async uploadArchive(app, archivePath, gitHash) {
    ...
    form.append("sourceFile", (0, import_node_fs.createReadStream)(archivePath));
